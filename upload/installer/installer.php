...
require_once('../includes/global_func.php');
...
function install()
{
    ...
    $c = ($GLOBALS["___mysqli_ston"] = mysqli_connect($db_hostname,  $db_username,  $db_password));
    ((bool)mysqli_query( $c, "USE " . $db_database));
    // Done, move on
    echo '... Successful.<br />';
    ...
    $config_file =
            <<<EOF
<?php
\$c = new mysqli('{$e_db_hostname}', '{$e_db_username}', '{$e_db_password}', '{$e_db_database}');

if(\$c->connect_errno > 0){
    die('Unable to connect to database [' . \$c->connect_error . ']');
}
EOF;
    $f = fopen('../includes/mysql.php', 'w');
    fwrite($f, $config_file);
    fclose($f);
    echo '... file written.<br />';
    echo 'Writing base database schema...<br />';
    $fo = fopen("includes/dbdata.sql", "r");
    $query = '';
    $lines = explode("\n", fread($fo, 1024768));
    fclose($fo);
    ...
    mysqli_query( $c, 
            "INSERT INTO `users`
             ...
             '{$adm_gender}', " . time()
                    . ", '{$ins_email}', -1, '$IP',
             '{$e_salt}')") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
