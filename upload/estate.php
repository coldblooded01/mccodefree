...
$h->menuarea();
$mpq = mysqli_query("SELECT * FROM houses WHERE hWILL={$ir['maxwill']}", $c);
$mp = mysqli_fetch_array($mpq);
$_GET['property'] = abs((int) $_GET['property']);
if ($_GET['property'])
{
    $npq =
            mysqli_query($c,"SELECT * FROM houses WHERE hID={$_GET['property']}");
...
{
    $npq =
            mysqli_query($c,"SELECT * FROM houses WHERE hWILL={$ir['maxwill']}");
    $np = mysqli_fetch_array($npq);
    if ($ir['maxwill'] == 100)
    {
