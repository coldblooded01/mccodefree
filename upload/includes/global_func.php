...
function item_dropdown($connection, $ddname = "item", $selected = -1)
{
    $ret = "<select name='$ddname' type='dropdown'>";
    $q =
            mysqli_query($connection,"SELECT * FROM items ORDER BY itmname ASC");
    if ($selected == -1)
    {
        $first = 0;
    }
...
function location_dropdown($connection, $ddname = "location", $selected = -1)
{
    $ret = "<select name='$ddname' type='dropdown'>";
    $q =
            mysqli_query($connection,"SELECT * FROM cities ORDER BY cityname ASC");
    if ($selected == -1)
    {
        $first = 0;
    }
...
function shop_dropdown($connection, $ddname = "shop", $selected = -1)
{
    $ret = "<select name='$ddname' type='dropdown'>";
    $q =
            mysqli_query($connection,"SELECT * FROM shops ORDER BY shopNAME ASC");
    if ($selected == -1)
    {
        $first = 0;
    }
...
function user_dropdown($connection, $ddname = "user", $selected = -1)
{
    $ret = "<select name='$ddname' type='dropdown'>";
    $q =
            mysqli_query($connection,"SELECT * FROM users ORDER BY username ASC");
    if ($selected == -1)
    {
        $first = 0;
    }
...
function event_add($userid, $text, $connection)
{
    ...
    mysqli_query(
            $connection, 
            "INSERT INTO events VALUES(NULL,$userid," . time() . ",0,'$text')") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
    return 1;
}
...
{
    global $c;
    return mysqli_real_escape_string($str, $c);
}
